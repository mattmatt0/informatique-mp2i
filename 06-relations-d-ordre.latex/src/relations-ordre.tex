...
\begin{document}
	...
	\section{Relations}
		...
		\subsection{Relations binaires}
			...
			\prop{Soit $X$ un ensemble non vide. Soit $Y$ un sous-ensemble non vide de $X$.
			Si $\mathcal{R}$ est une relation binaire sur $X$, alors $\mathcal{R}' = \mathcal{R} \cap Y^2$
			est une relation binaire sur $Y$ qui a les mêmes propriété de transitivité, réflexivité, symétrie, antisymétrie que $\mathcal{R}$.}
			\begin{demo}
				\item En exercice.
			\end{demo}
			...
			\prop{$\mathcal{R}_R$ est la plus petite relation réflexive contenant $\mathcal{R}$ (au sens de l'inclusion en tant qu'ensemble).}
			\begin{demo}
			...
			\prop{Soit $n \in \mathbb{N}$. Soit $\mathcal{R}$ une relation binaire sur $X$. 
				On a $x \mathcal{R}^n y \Leftrightarrow \exists (x_0,...x_n) \in X^{n+1}$ avec $x_0 = x, x_n = y$, 
				et $\forall k \in \intsete{0,n}$, $x_k \mathcal{R} x_{k+1}$.}
			\begin{demo}
			...
			\prop{$\mathcal{R}_T$ est la plus petite relation transitive contenant $\mathcal{R}$.}
			\begin{demo}
	...
	\section{Ordre}
		...
		\subsection{Ordre bien fondé}
			...
			\prop{Si $\leq$ est une relation d'ordre totale sur $X$ (autrement dit si $(X,\leq)$ est un ordre total),
			...
			\prop{Un ordre est bien fondé ssi il n'existe pas une suite infinie strictement décroissante.}
			\begin{demo}
