...
\begin{document}
	...
	\begin{center}
		\textit{Dans ce chapitre, on présente différents paradigmes algorithmiques qui permettent de développer des algorithmes résolvant des problèmes (souvent d'optimisation) mettant en jeu des textes, des nombres, des listes, des graphes, des expressions arithmétiques, des points du plan, des ordonnancements...}
		\vspace{4mm}
	\end{center}
	
	\noindent \elt{Rappel :} On rappelle qu'un problème d'optimisation consiste à minimiser une fonction à valeurs réelles données sur un ensemble de solutions donné.
		\vspace{2.5mm}
		%
		% \\[5mm]
		%
	\begin{thmbox}[S]{\elt{\underline{Définition :}}} Ainsi, un tel problème $\mathcal{P}$ est défini par la fonction dite fonction-objectif $f$ et l'ensemble des solutions $\mathcal{S}$. \\
	On note
		\( \displaystyle \mathcal{P} : \max_{X \in \, \mathcal{S}} f(X) \) (ou bien \( \displaystyle \mathcal{P} : \min_{X \in \, \mathcal{S}} f(X) \) s'il s'agit de minimiser $f$). \\
	On appelle alors valeur du problème $\mathcal{P}$ :
		\( \mathrm{val} (\mathcal{P}) = \max \, \! \left\{ f(X) \ \middle| \, X \in \mathcal{S} \right\} \). \\
	On note aussi
		\( \displaystyle \underset{X \in \, \mathcal{S}}{\mathrm{argmax}} \: f(X) = \left\lbrace X \in \mathcal{S} \ \middle| \, f(X) = \mathrm{val} (\mathcal{P}) \right\rbrace \) l'ensemble des solutions optimales. \end{thmbox}
		%
		% \\[5mm]
		%
	\elt{Remarque :} L'unicité de la valeur optimale n'entraîne pas celle des solutions optimales.
		%
		\\[5mm]
		%
	\elt{Remarque :} Si \( \mathcal{S} \subseteq \widetilde{\mathcal{S}} \subseteq \mathcal{D} (f)\), on dit que \( \widetilde{\mathcal{P}} = \displaystyle \max_{X \in \, \mathcal{S}} f(X) \) est un relâché de $\mathcal{P}$. \\
	...
	\section{Algorithmes gloutons}
		...
		\subsection{L'exemple du problème du sac à dos}
			...
			\elt{Remarque :} \textsf{\textit{Sac à dos fractionnaire}} est un relâché de \textsf{\textit{Sac à dos}}.
				%
				\\[5mm]
				%
			\elt{Remarque :} La contrainte $(*)$ s'écrit aussi/se traduit aussi par : \hspace{-4.2mm}
				\begin{tabular}{l}
			...
			\end{tabular}
				\nl
			\elt{Remarque :} Les exemples précédents montrent que pour le problème \textsf{\textit{Sac à dos}}, les stratégies simples consistant à prendre les objets simplement par $v_i$ décroissants, $p_i$ croissants ou $v_i/p_i$ décroissants ne sont pas optimales. En fait, \textsf{\textit{Sac à dos}} est un problème difficile $^{\text{(?)}}$.
				\\[2mm]
			...
			\renewcommand{\arraystretch}{1}
				...
				\begin{tabular}{|| l}
						\parbox{150mm}{
							...
							\\
							$k = 1$
							\begin{thmbox}[M]{Tant que $k \leq n$ et $s + p_{\sigma(k)} \leq P$ \hspace*{4mm} $/ \hspace{-1mm} /$ \textit{Invariant} : \( \textstyle s = \sum_{i=1}^{n} \delta_i p_i \)}
			...
			On montre l'optimalité de cette stratégie par un argument d'échange.
				\\[0mm]
				%
			\begin{thmbox}[S]{\elt{\underline{Propriété :}}}
				Soit $n \in \bb{N}^*$. \\
			...
			%
		\pagebreak
		\begin{thmbox}[S]{\elt{\underline{Corollaire :}}}
	...
	\subsection{Le problème du tri}
		...
		\textsf{\textbf{\underline{TRI}}} \hspace{-0.6mm}
			...
			\end{tabular}
			\nl
		\elt{Remarque :} \textsf{\textit{Tri}} est un problème d'optimisation, il consiste en la recherche de :
			\vspace{-3.8mm}
			\[
		...
		\begin{tabular}{|| l}
			\parbox{150mm}{
				...
				\begin{thmbox}[M]{Pour $k$ allant de 1 à $n$}
		...
		\end{tabular}
			\nl
			%
		\elt{Remarque :} Cet algorithme peut être optimisé : en effet, si on avait directement initialisé $\sigma$ au tableau identité, on n'aurait pas eu besoin du tableau $I$.
			\\[-0mm]
			%
		\begin{thmbox}[S]{\underline{\elt{Propriété :}}}
	...
	\subsection{À retenir sur les algorithmes gloutons}
		...
		\vspace{2mm}
		\begin{thmbox}[S]{\elt{\underline{Définition :}}}
		...
		\elt{Exemples :} L'algorithme de Huffman (\emph{cf.} DM n°3 - Partie 2/2) et l'algorithme de résolution du problème du \textsf{\textit{Sac à dos fractionnaire}} sont des algorithmes gloutons.
			%
			\nl
		\elt{Remarque :} Attention, selon les problèmes, un même algorithme peut être optimal ou non. En général, les algorithmes gloutons sont efficaces (faible complexité, a fortiori polynomiale) : ils ne sont donc pas exacts pour les problèmes difficiles/NP-complets (comme \textsf{\textit{Sac à dos entier}}).
			\\[2mm]
		Néanmoins, ils peuvent être utiles pour obtenir rapidement la borne supérieure ou inférieure, \emph{c-à-d} un majorant ou un miniorant de la valeur optimale.
			%
			\nl
		\elt{Remarque :} Pour savoir si un problème d'optimisation peut être résolu par un algorithme glouton, on se pose deux questions : \\
			\hspace*{4mm}
		\parbox[t]{164mm}{
...
\section{Programmation dynamique}
	...
	\subsection{Plus long sous-mot commun}
		...
		\end{center}
			%
			\vspace{2mm}
		\begin{thmbox}[S]{\elt{\underline{Rappel :}}}
