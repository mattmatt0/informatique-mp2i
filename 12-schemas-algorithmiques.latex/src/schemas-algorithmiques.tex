...
\begin{document}
	...
	\notecentrale{Dans tout ce chapitre, $\bbm{1}_S$ désigne le $n$-uplet 
	...
	Ainsi $\bbm{1}_{\set{1,3,4}} = (0,1,0,1,1,0,0...)$. 
	On écrira $\bbm{1}_i$ pour $\bbm{1}_{\set{i}}$ et $\bbm{1}$ pour $\bbm{1}_{\intset{1,n}}$.}
	...
	\section{Algorithmes gloutons}
		\subsection{L'exemple du problème du sac à dos}
			...
			\rem La contrainte $(*)$ se traduit aussi par  $\forall i \in \intset{1,n}, 1 \geq \delta \cdot \bbm{1}_i \geq 0 $.
			En effet,
			\[
				\forall i \in \intset{1,n}, \delta \cdot \1_{i} = 
				\sum_{j=1}^{n} \delta_j \times \bbm{1}_{ij} = 
				\delta_i \times \bbm{1}_{ii} = 
				\delta_i \times 1 = 
				\delta_i 
			\].
