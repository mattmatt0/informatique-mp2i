		...
		\subsection{Opérations en pseudo-code}
			...
			\begin{algorithm}[H]
				...
				\Sortie
				{
					$t'$ un ABPT vérifiant 
					\begin{itemize}
						\item $\card(\set{c \in \mathrm{ch}(t') \tq \left(\mathrm{etiq}(t')\right)(c) = e}) = \card(\set{c \in \mathrm{ch}(t') \tq \left(\mathrm{etiq}(t')\right)(c) = e}) + 1$ 
						\item $\forall x \in E\setminus\set{e}, \card(\set{c \in \mathrm{ch}(t') \tq \left(\mathrm{etiq}(t')\right)(c) = x}) = \card(\set{c \in \mathrm{ch}(t') \tq \left(\mathrm{etiq}(t')\right)(c) = x})$
					\end{itemize}
				}
