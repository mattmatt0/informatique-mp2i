\newcommand\PATH{/home/matthieu/Documents/Ecole/Informatique/cours/informatique-mp2i/08-arbres.latex}
...
\begin{document}
	...
	\section{Arbres 2-3-4}
		...
		\subsection{Insertion dans un arbre 2-3-4}
			Pour insérer une clé $e \in \mathcal{S}$ dans $t$ un arbre 2-3-4, on procède comme suit : 
			\begin{itemize}
				\item Si $n$ est une feuille, 
			\end{itemize}
		\subsection{Scission d'un 4-noeud}
			...
			\begin{center}
				...
				level 3/.style={sibling distance=2em}]
					\node [circle,draw](a) {4}
						child {node [ellipse,draw] (b) {$a \leq b \leq c$}
							child {node (c) {$\times$}}
							child {node (c) {$\times$}}}
						child {node [circle,draw] (b) {3}
							child {node [circle,draw] {4}
							...
							child {node (d) {$\times$}}};
				\end{tikzpicture} \\
				\textsc{Scission d'un 4-noeud racine}
			\end{center}
