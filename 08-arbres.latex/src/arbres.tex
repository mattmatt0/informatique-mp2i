...
\begin{document}
	...
	\section{Arbre binaires}
		...
		\subsection{Vocabulaire}
			...
			si et seulement si il existe $g \in \mathcal{A}_B(\mathcal{S})$ (resp. $d \in \mathcal{A}_B(\mathcal{S})$) tel que $t = N(x,g,t')$ (resp. $t = N(x,t',d)$).
		...
		\subsection{Hauteur}
			On définit par induction la hauteur $h(t)$ d'un arbre binaire $t \in \mathcal{A}_B(\mathcal{S})$ comme valant 
			-1 si $t = V$ et $1 + \max(h(g),h(d))$ si $t = N(x,g,d)$.

			\prop{Soit $t \in \mathcal{A}_B(\mathcal{S}) \setminus \set{V}$. 
			...
			\begin{demo}
				...
				\item Soit $t = N(x,g,d) \in \mathcal{A}_B(\mathcal{S})$ avec $g$ et $d$ deux arbres binaires non vides vérifiant la propriété.
					On a 
					\[
			...
			\prop{Pour $t \in \mathcal{A}_B(\mathcal{S})$, on a $h(t) + 1 \leq s(t) \leq 2^{h(t)+1}-1$.}
			\begin{demo}
				\item Montrons-le par induction sur $t$ : pour $t = V$, on a $h(t) + 1= 0$, $s(t) = 0$ et $2^{h(t)+1}-1 = 0$.
				\item Soit $t = N(x,g,d) \in \mathcal{A}_B(\mathcal{S})$, où $g$ et $d$ respectent la propriété énoncée.
					Alors 
					\[
