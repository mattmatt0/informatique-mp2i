...
\begin{document}
	...
	\section{Arbre binaires}
		...
		\subsection{Parcours}
			...
			est dit \textbf{préfixe} (resp. postfixe, infixe) si et seulement si pour tout $n \in \mathrm{ch}(t)$, pour tout $g \in \mathscr{G}(n)$ et $d \in \mathscr{D}(n)$,
			on a $\varphi(n) \leq \varphi(g) \leq \varphi(d)$ (resp. $\varphi(g) \leq \varphi(d) \leq \varphi(n)$ et $\varphi(g) \leq \varphi(n) \leq \varphi(d)$).

			\exemple Pour l'arbre suivant : 
			\begin{center}
				\begin{tikzpicture}
					\draw (0,0) circle(.3) node {1};
					\draw (0,-1) circle(.3) node {2};
					\draw (0,-1) circle(.3) node {36};
					\draw (0,0) circle(.3) node {1};
				\end{tikzpicture}
			\end{center}
